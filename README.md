...
In order to clear tasks, it is possible to purge tasks that are lower then a given sequence number from a repository. This allows to clear out tasks after they are no longer needed. Finally, it is possible to resolve any given `LocalDateTime` to the first sequence number that is assigned after it. Using this mechanism, it is possible to select tasks within date ranges rather then by a sequence number. Specified dates are always synchronized with the database clock to avoid accidental mismatches.  
...
Function<String, TaskConsumer<TRANSACTION, EXCEPTION>> factory = new CompositeTaskHandlerFactory<>(
  ...
  (task, context) -> { System.out.println("Running step 1 for " + task); }
).withSimpleSingularSyncStep(
  "step2", // The name of the second step
  (task, context) -> { System.out.println("Running step 2 for " + task); },
  "step1" // Determines that this second step depends on the first step
);
```

A `CompositeTaskHandlerFactory` provides access to a user-defined `TaskContext` to all of its steps. On completing a series of steps, the context is then responsible to make any final adjustments to the resulting tasks and to contribute to the committing transaction. As a suggestion for an implementation, the artifact also implements a `CompositeTaskContext`, which combines several task contexts and also serves as a `TaskSupplement` which allows for concurrent registration and consumption of task metadata.
